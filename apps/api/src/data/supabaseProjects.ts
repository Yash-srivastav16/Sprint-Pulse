...
const dateKeyFromNow = (daysFromNow: number) => {
  const date = new Date();
...
export const connectSupabaseJiraProject = async (
  request: JiraConnectRequest
): Promise<JiraConnectResponse | undefined> => {
  const viewerProfile = await findSupabaseProfileById(request.personaId);
  if (!viewerProfile) {
    return undefined;
  }
  ...
  const client = requireSupabaseAdmin();
  const key = request.projectKey.trim().toUpperCase();
  const jiraSite = normalizeJiraSite(request.jiraSite);
  if (!key || !jiraSite) {
  ...
  let projectRow = existingProject as ProjectRow | null;
  if (projectRow && projectRow.created_by !== viewer.id) {
    return undefined;
  }
  ...
  if (!sprintRow) {
    const { data: sprint, error: sprintError } = await client
      ...
      .insert({
        project_id: projectRow.id,
        name: `${key} Active Sprint`,
        goal: "Imported from Jira. Connect Atlassian OAuth and sync issues to hydrate live sprint signal.",
        start_date: dateKeyFromNow(0),
        end_date: dateKeyFromNow(13),
        status: "active",
        created_at: now,
        updated_at: now
      })
  ...
  const { error: memberError } = await client.from("project_members").upsert(creatorMember);

  if (memberError) {
    throw new Error(memberError.message);
  }

  const { error: connectionError } = await client.from("jira_connections").upsert(
    {
      project_id: projectRow.id,
      site_url: jiraSite,
      project_key: key,
  ...
  return {
    project: sprintProject,
    importedIssues: 0,
    importedMembers: sprintProject.members.length,
    importedAt: now,
    warnings: ["Jira project details are saved. Connect Atlassian OAuth from Integrations, then sync live issues."]
  };
};
