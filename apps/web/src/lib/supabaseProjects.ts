...
export const connectJiraProjectInSupabase = async (input: JiraConnectRequest): Promise<JiraConnectResponse> => {
  const client = requireSupabase();
  const key = input.projectKey.trim().toUpperCase();
  ...
  const created = await createProjectInSupabase({
    personaId: input.personaId,
    projectName: `${key} Jira Project`,
    projectKey: key,
    sprintName: `${key} Active Sprint`,
    sprintGoal: "Imported from Jira. Connect Atlassian OAuth and sync issues to hydrate live sprint signal.",
    startDate: dateKeyFromNow(0),
    endDate: dateKeyFromNow(13),
    members: []
  });
  const now = new Date().toISOString();
  const projectWrite = await client
    .from("projects")
    .update({
      source: "jira",
      jira_site: jiraSite,
  ...
  const detail = await getProjectFromSupabase(created.project.id, input.personaId);

  return {
    project: detail.project,
    importedIssues: 0,
    importedMembers: detail.project.members.length,
    importedAt: now,
