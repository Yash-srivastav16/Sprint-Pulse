...
import { MemberAvatar } from '@/components/workspace/WorkspaceChrome';
...
export function Sidebar({ mobileOpen = false, onClose }: SidebarProps) {
  ...
  }, [clearProject, navigate, persona?.id, selectedProjectId]);
  ...
  }, [persona?.id, selectedSprintId, visibleProjectId]);
    ...
    <>
      ...
      <div className="relative border-b border-white/10 p-5">
        <div className="flex items-center justify-between gap-3">
          <Link to={visibleProjectId ? `${projectBase}/dashboard` : '/projects'} className="flex items-center gap-3 group" onClick={isMobile ? onClose : undefined}>
            <motion.div
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary-200">Sprint risk intelligence</p>
            </div>
          </Link>
          {isMobile ? (
            <button
      ...
      <div className="relative space-y-3 border-b border-white/10 p-4">
        <div className="space-y-1">
          <label className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500" htmlFor={isMobile ? "project-switcher-mobile" : "project-switcher"}>
            Project
          </label>
          <div className="relative">
            <select
              id={isMobile ? "project-switcher-mobile" : "project-switcher"}
              value={visibleProjectId ?? ''}
              onChange={(event) => handleProjectChange(event.target.value)}
              className="h-11 w-full appearance-none rounded-2xl border border-white/10 bg-white/[0.07] px-3 pr-9 text-sm font-bold text-white outline-none transition focus:border-primary-300/70 focus:bg-white/[0.1] focus-visible:ring-2 focus-visible:ring-primary-300/50 [&>option]:bg-slate-900 [&>option]:text-white"
            >
        ...
        {visibleProjectId ? (
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-500" htmlFor={isMobile ? "sprint-switcher-mobile" : "sprint-switcher"}>
              Sprint
            </label>
            <div className="relative">
              <select
                id={isMobile ? "sprint-switcher-mobile" : "sprint-switcher"}
                value={activeSprint?.id ?? ''}
                onChange={(event) => handleSprintChange(event.target.value)}
                className="h-11 w-full appearance-none rounded-2xl border border-white/10 bg-white/[0.07] px-3 pr-9 text-sm font-bold text-white outline-none transition focus:border-info-300/70 focus:bg-white/[0.1] focus-visible:ring-2 focus-visible:ring-info-300/50 [&>option]:bg-slate-900 [&>option]:text-white"
              >
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.065] p-3 shadow-glass backdrop-blur">
          ...
          <p className="mt-1 truncate text-xs text-slate-400">
            {visibleProjectId ? activeSprint?.name ?? project?.sprintName ?? 'Select a sprint to unlock signals' : 'Choose a project to unlock delivery pages'}
          </p>
        </div>
      ...
      <nav className="relative min-h-0 flex-1 space-y-2 overflow-y-auto p-4">
        {navigation.map((item) => (
          <NavLink key={item.name} to={item.href} end={item.end} onClick={isMobile ? onClose : undefined}>
            {({ isActive }) => (
              <motion.div
                whileHover={{ x: 4 }}
                whileTap={{ scale: 0.98 }}
              ...
              >
                {isActive ? (
                  <motion.span
                    layoutId={isMobile ? "sidebar-active-rail-mobile" : "sidebar-active-rail"}
        ...
        {!visibleProjectId ? (
          <div className="mt-4 rounded-2xl border border-dashed border-white/12 bg-white/[0.04] p-4 text-sm leading-6 text-slate-400">
            ...
            Choose a project to open Dashboard, Standups, Team, Sprints, and Integrations.
          </div>
        ) : null}
      </nav>
      ...
      <div className="relative shrink-0 border-t border-white/10 bg-slate-950/35 p-4 backdrop-blur">
        <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.065] p-3 shadow-glass">
          <MemberAvatar
            initials={persona?.initials || user?.email?.charAt(0).toUpperCase() || 'SP'}
            seed={persona?.name || user?.email || 'SprintPulse user'}
            size="sm"
          />
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-400 truncate">{persona?.title || 'Team member'}</p>
          </div>
          <button
            aria-label="Sign out of SprintPulse"
            title="Sign out"
            className="grid h-8 w-8 place-items-center rounded-xl text-slate-400 transition-colors hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-300/60"
            type="button"
        ...
        {mobileOpen ? (
          <>
            ...
            <motion.aside
              ...
              className="fixed inset-y-0 left-0 z-40 flex w-80 flex-col overflow-hidden border-r border-white/10 bg-gradient-to-b from-[#060a13] via-[#0d1422] to-[#111a2b] text-white shadow-2xl lg:hidden"
            >
              {sidebarContent(true)}
            </motion.aside>
          </>
