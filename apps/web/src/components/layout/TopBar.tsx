    ...
    >
      <div className="h-full px-4 sm:px-6 flex items-center justify-between gap-4">
        ...
        <div className="flex min-w-0 flex-1 justify-center px-3">
          {project ? (
            <div className="hidden min-w-0 max-w-xl rounded-full border border-slate-200/80 bg-white/70 px-4 py-2 text-xs font-semibold text-slate-600 shadow-sm dark:border-white/10 dark:bg-white/5 dark:text-slate-300 lg:block">
              <span className="text-primary-600 dark:text-primary-300">{project.projectName}</span>
              <span className="mx-2 text-slate-300">/</span>
              <span className="truncate">{project.sprintName}</span>
            </div>
          ) : null}
        </div>
