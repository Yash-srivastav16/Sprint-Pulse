...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const upsertSupabaseProfile = async (input: {
    ...
    const row: ProfileRow = {
      id: existingProfile?.id ?? (slugFromEmail(input.email) || `user-${Date.now()}`),
  ...
  useEffect(() => {
    ...
    const {
      ...
      setIsLoading(true);
      window.setTimeout(() => {
        withTimeout(runApply(nextSession), AUTH_PROFILE_TIMEOUT_MS + AUTH_BACKGROUND_TIMEOUT_MS, "Auth session")
          .catch(() => {
            setSession(null);
            setUser(null);
            clearPersona();
  ...
  const value = useMemo<AuthContextValue>(
    () => ({
      ...
      session,
      user,
      isAuthenticated: Boolean(persona) && !supabaseConfigError,
      isLoading,
