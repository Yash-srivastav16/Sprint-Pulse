    ...
    >
      <section className="premium-surface relative rounded-2xl p-6">
        ...
        <div className="relative grid items-stretch gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
          ...
          <Card className="overflow-hidden rounded-2xl border-slate-200/80 bg-white/92 text-slate-950 shadow-[0_18px_60px_rgba(15,23,42,0.10)] dark:border-white/10 dark:bg-white/[0.055] dark:text-white">
            <div className="h-1.5 bg-gradient-to-r from-info-500 via-primary-400 to-ai-500" />
            <CardContent className="grid gap-5 p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="m-0 text-xs font-black uppercase tracking-[0.14em] text-info-700 dark:text-info-100/80">Connection preview</p>
                  <h2 className="m-0 mt-1 text-xl font-black tracking-normal text-slate-950 dark:text-white">{projectKey.trim() || "Project key"}</h2>
                  <p className="m-0 mt-1 break-all text-sm font-semibold text-slate-600 dark:text-slate-300">{previewUrl}</p>
                </div>
                <span className="grid h-11 w-11 place-items-center rounded-2xl border border-info-500/20 bg-info-500/10 text-info-700 dark:border-white/10 dark:bg-white/10 dark:text-info-100">
                  <Cloud className="h-5 w-5" />
                </span>
              </div>
              <div className="grid gap-3 rounded-2xl border border-slate-200/80 bg-slate-50/80 p-4 dark:border-white/10 dark:bg-white/[0.075]">
                {[
                  ["Site", siteReady],
                  ["Key", keyReady],
                  ...
                  <span className="flex items-center justify-between gap-3 text-sm" key={label as string}>
                    <span className="font-bold text-slate-500 dark:text-slate-300">{label as string}</span>
                    <span className={cn("inline-flex items-center gap-2 rounded-full px-2.5 py-1 text-xs font-black", ready ? "bg-primary-500/10 text-primary-700 dark:bg-primary-300/15 dark:text-primary-100" : "bg-slate-100 text-slate-400 dark:bg-white/[0.065]")}>
                      {ready ? <CheckCircle2 className="h-3.5 w-3.5" /> : <span className="h-3.5 w-3.5 rounded-full border border-slate-300 dark:border-white/20" />}
                      {ready ? "Ready" : "Needed"}
                    </span>
                  </span>
                ))}
              ...
              <div className="grid grid-cols-3 gap-2 text-center text-xs font-black text-slate-600 dark:text-slate-300">
                <span className="rounded-2xl border border-slate-200 bg-slate-50 px-2 py-3 dark:border-white/10 dark:bg-white/[0.055]">Sprint</span>
                <span className="rounded-2xl border border-slate-200 bg-slate-50 px-2 py-3 dark:border-white/10 dark:bg-white/[0.055]">Issues</span>
                <span className="rounded-2xl border border-slate-200 bg-slate-50 px-2 py-3 dark:border-white/10 dark:bg-white/[0.055]">Members</span>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <aside className="premium-surface grid gap-4 self-start rounded-2xl p-5">
          ...
          <div className="grid gap-3">
            {[
              ...
              [ShieldCheck, "OAuth follows next", "Authorize Atlassian from Integrations when you are ready to sync live issues."]
