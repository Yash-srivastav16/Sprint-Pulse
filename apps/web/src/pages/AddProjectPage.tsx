          ...
          <Card className="overflow-hidden rounded-2xl border-slate-200/80 bg-white/92 text-slate-950 shadow-[0_18px_60px_rgba(15,23,42,0.10)] dark:border-white/10 dark:bg-white/[0.055] dark:text-white">
            ...
            <CardContent className="grid gap-5 p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="m-0 text-xs font-black uppercase tracking-[0.14em] text-primary-700 dark:text-primary-100/80">Workspace preview</p>
                  <h2 className="m-0 mt-1 text-xl font-black tracking-normal text-slate-950 dark:text-white">{projectName.trim() || "Project name"}</h2>
                  <p className="m-0 mt-1 text-sm font-semibold text-slate-600 dark:text-slate-300">{projectKeyPreview}</p>
                </div>
              ...
              <div className="grid gap-3 rounded-2xl border border-slate-200/80 bg-slate-50/80 p-4 dark:border-white/10 dark:bg-white/[0.075]">
                <span className="flex items-center justify-between gap-3 text-sm">
                  <span className="font-bold text-slate-500 dark:text-slate-300">Sprint</span>
                  <strong className="text-right text-slate-950 dark:text-white">{sprintName.trim() || "Sprint name"}</strong>
                </span>
                <span className="flex items-center justify-between gap-3 text-sm">
                  <span className="font-bold text-slate-500 dark:text-slate-300">Window</span>
                  <strong className="text-right text-slate-950 dark:text-white">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <span className={cn("grid min-h-16 place-items-center rounded-2xl border px-2 text-center text-xs font-black", ready ? "border-primary-500/25 bg-primary-500/10 text-primary-700 dark:border-primary-300/30 dark:bg-primary-300/15 dark:text-primary-100" : "border-slate-200 bg-slate-50 text-slate-400 dark:border-white/10 dark:bg-white/[0.055]")} key={label as string}>
                    {ready ? <CheckCircle2 className="h-4 w-4" /> : <span className="h-4 w-4 rounded-full border border-slate-300 dark:border-white/20" />}
                    {label as string}
                  </span>
                ))}
              </div>
